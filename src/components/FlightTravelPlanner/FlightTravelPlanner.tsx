  ...
  return (
    <Wrapper
      ...
      <Container>
        ...
        >
          <div className="grd-flex @md:grd-items-center grd-flex-col @md:grd-flex-row">
            <ButtonGroup>
            ...
            <div className="grd-flex @md:grd-items-center grd-gap-2 @md:grd-gap-8 @md:grd-ml-auto grd-font-medium grd-text-sm grd-select-none grd-mt-4 @md:grd-mt-0 grd-flex-col @md:grd-flex-row">
              {isAgency && (
                <div
                  style={{
                    color: strategy.data?.buttonStyle?.borderColor || design?.button?.borderColor
          ...
          <div className={`grd-w-full grd-grid grd-group grd-items-center grd-grid-cols-12 grd-gap-4 ${isAgency ? ActiveTab === "multi" ? " @md:grd-grid-cols-4" : " @md:grd-grid-cols-5" : " @md:grd-grid-cols-4"}`}>
            {Object.values(FlightAmounts).map((flight: any, index, ) => {
              const RotationChange = flight.itemRotation
              return (
                <div className={`grd-w-full grd-grid grd-items-center grd-gap-4 grd-col-span-12 ${ActiveTab === "return" ? "@md:grd-col-span-3 @md:grd-grid-cols-3" : "@md:grd-col-span-4 @md:grd-grid-cols-4"}`} key={Math.random()} >
                  {ActiveTab === "multi" && (
                    <div className="multiple-flight-title-box grd-grid-cols-12 @md:grd-col-span-4">
                      <p className="grd-text-black grd-font-semibold grd-text-sm -grd-mb-2 dark:grd-text-white">{index + 1}.Uçuş</p>
                    </div>
                  )}
                  <div className={`grd-relative grd-flex grd-gap-4 grd-flex-col @md:grd-flex-row grd-grid-cols-12${ActiveTab !== "multi" ? (ActiveTab === "one-way" ? " @md:grd-col-span-3" : " @md:grd-col-span-2") : " @md:grd-col-span-3"}`}>
                    ...
                    <div
                      ...
                      className={`grd-absolute grd-top-1/2 @lg:grd-left-1/2 grd-right-0 @lg:grd-right-auto -grd-translate-x-1/2 -grd-translate-y-1/2 grd-rotate-90 @lg:grd-rotate-0 grd-group-has-[input] grd-p-2 grd-cursor-pointer grd-text-gray-500 grd-transition-all 
            ...
            {ActiveTab === "multi" && !isAgency && (
              <div className="grd-flex grd-gap-2 grd-grid-cols-12 grd-col-span-12 @md:grd-col-span-4">
                <div
            ...
            {ActiveTab === "return" && <Input
						  className="grd-col-span-12 @md:grd-col-span-1"
						  leftIcon={<LIcon size={18} name="Calendar" />}
						  style={{borderRadius: design?.borderRadius}}
            ...
            {!isAgency && (
              <>
                ...
                <div className="grd-col-span-12 grd-flex-col @md:grd-flex-row @md:grd-col-span-2 grd-flex @md:grd-grid-cols-3 grd-gap-4">
                  <Input
                    className={noReason ? "grd-col-span-12 @md:grd-col-span-2" : ""}
                    rightIcon={<LIcon size={18} name="ChevronDown"/>}
                    style={{borderRadius: design?.borderRadius}}
            ...
            {ActiveTab === "multi" && isAgency && (
              <Button
                onClick={() => addFlight(`flight_${Object.values(FlightAmounts).length}`)}
                className={`grd-w-full group-[.is-disabled]:grd-cursor-not-allowed grd-col-span-12 @md:grd-col-span-1`}
